          ...
          <div key={i} className={`${styles.msg} ${styles[`msg_${msg.role}`]}`}>
            <div className={styles.bubble}>
              ...
              {msg.retrieved_chunks && msg.retrieved_chunks.length > 0 && (
                <details className={styles.sourceChunks}>
                  <summary>📄 Retrieved Source Chunks (In Reranked Order)</summary>
                  <div className={styles.chunksList}>
                    {msg.retrieved_chunks.map((chunk, idx) => (
                      <div key={idx} className={styles.chunkItem}>
                        <strong>#{idx + 1} {chunk.metadata.source} (Page {chunk.metadata.page})</strong>
                        <p>{chunk.text}</p>
                        <small>RRF: {chunk.rrf_score?.toFixed(3)} | Dense: {chunk.dense_score?.toFixed(3)} | Sparse: {chunk.sparse_score?.toFixed(3)}</small>
                      </div>
                    ))}
                  </div>
                </details>
